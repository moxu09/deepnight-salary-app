...
function isWalletPayment(value) {
  const text = String(value || "");
  return (
    text.includes("儲值卡") || text.includes("錢包") || text.includes("餘額")
  );
}

function isMonthlyPayment(value) {
  return String(value || "").includes("月結");
}
...
function makeMonthRange(monthText) {
  ...
  const nowYear = Number(nowParts.find((part) => part.type === "year")?.value);
  const nowMonth = Number(
    nowParts.find((part) => part.type === "month")?.value
  );
  const year = matched ? Number(matched[1]) : nowYear;
  const month = matched ? Number(matched[2]) : nowMonth;
...
function orderRows(order) {
  ...
  const finalAmount = numberValue(
    order.final_price || order.price || order.order_amount || grossAmount
  );
  const discountAmount =
    numberValue(order.discount_amount) ||
    Math.max(0, grossAmount - finalAmount);
  const staffSalary = numberValue(order.staff_salary);
  const bonusAmount = numberValue(order.bonus_amount);
  const paymentMethod = order.payment_method || "";
  const walletPayment = isWalletPayment(paymentMethod);
  const monthlyPayment = isMonthlyPayment(paymentMethod);
  const orderNo = order.order_no || order.order_id || String(order.id);
  const service =
    order.service_name || order.service || order.order_item || "陪玩訂單";
  const rows = [];
  ...
  if (finalAmount !== 0) {
    rows.push(
      makeRow({
        ...
        subject: "服務收入",
        amount: finalAmount,
        cash_amount: walletPayment || monthlyPayment ? 0 : finalAmount,
        revenue_amount: finalAmount,
        liability_amount: walletPayment ? -finalAmount : 0,
        receivable_amount: monthlyPayment ? finalAmount : 0,
        payment_method: paymentMethod,
        customer_id: order.customer_id,
        customer_name: order.customer_name || order.customer_username,
        staff_id: order.discord_id || order.assigned_player,
  ...
  if (discountAmount > 0) {
    rows.push(
      makeRow({
        ...
        order_no: orderNo,
        source: "play_orders",
        note: order.coupon_text || order.coupon_name || "優惠折抵",
      })
    );
  }
...
function bonusRow(bonus) {
  ...
  return makeRow({
    ...
    source: "players_bonus",
    note: [bonus.bonus_type, bonus.description].filter(Boolean).join("｜"),
  });
}

function withdrawRow(request) {
  const amount = numberValue(request.amount);
  const serviceFee = numberValue(request.service_fee);
  const welfareFee = numberValue(request.welfare_fee);
  const isAsdTransfer = request.destination === "asd";
  const payoutAmount =
    request.payout_amount === null || request.payout_amount === undefined
      ? amount - serviceFee - welfareFee
      : numberValue(request.payout_amount);

  return makeRow({
    id: `withdraw-${request.id}`,
    occurred_at:
      request.reviewed_at || request.updated_at || request.requested_at,
    category: isAsdTransfer ? "薪資轉 ASD" : "提領付款",
    subject: isAsdTransfer ? "轉入本人 ASD" : "薪資付款",
    amount,
    cash_amount: isAsdTransfer ? 0 : -payoutAmount,
    liability_amount: isAsdTransfer ? payoutAmount : 0,
    staff_id: request.discord_id,
    staff_name: request.staff_name,
    source: "salary_withdraw_requests",
    note: [
      request.request_note,
      welfareFee > 0 ? `福利金 ${welfareFee}` : null,
      serviceFee > 0 ? `手續費 ${serviceFee}` : null,
      isAsdTransfer ? `轉入 ASD ${payoutAmount}` : `實際匯款 ${payoutAmount}`,
    ]
      .filter(Boolean)
      .join("｜"),
  });
...
export function getAccountingRange(monthText) {
  return makeMonthRange(monthText);
}

export async function loadDeepnightAccountingReport(
  supabaseAdmin,
  { month, startIso, endIso }
) {
  const range =
    startIso && endIso ? { month, startIso, endIso } : makeMonthRange(month);
