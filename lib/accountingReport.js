...
function isWalletPayment(value) {
  ...
  return text.includes("儲值卡") || text.includes("錢包") || text.includes("餘額");
}
...
function makeMonthRange(monthText) {
  ...
  const nowMonth = Number(nowParts.find(part => part.type === "month")?.value);
...
function orderRows(order) {
  const occurredAt =
    order.order_finished_at || order.paid_at || order.completed_at || order.created_at;
  const grossAmount = numberValue(
    order.original_price || order.price || order.order_amount || order.final_price
  );
  const finalAmount = numberValue(
    order.final_price || order.price || order.order_amount || grossAmount
  );
  const discountAmount =
    numberValue(order.discount_amount) || Math.max(0, grossAmount - finalAmount);
  ...
  const service = order.service_name || order.service || order.order_item || "陪玩訂單";
  const rows = [];
