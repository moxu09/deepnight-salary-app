...
const TABLE_COLUMNS = [
  ...
  { key: "destination", label: "入帳方式", width: 66, align: "center" },
  { key: "service", label: "手續費", width: 62, align: "right" },
  { key: "payout", label: "實際入帳", width: 82, align: "right" },
...
function addPageHeader(doc, accent, company, from, to) {
  ...
  doc.font("Regular").fontSize(8).fillColor("#e2e8f0");
...
export async function createWithdrawalStatementPdf({
  ...
  drawSummaryCard(doc, 217, 226, 160, "扣除費用", money(approvedFees), accent);
  drawSummaryCard(doc, 394, 226, 161, "已完成實際入帳", money(approvedPayout), accent);
  ...
  if (requests.length === 0) {
    ...
    requests.forEach((request, index) => {
      ...
      const row = {
        ...
        destination: request.destination === "asd" ? "本人 ASD" : "銀行",
      ...
      for (const column of TABLE_COLUMNS) {
        ...
        doc
          ...
          .fillColor(isStatus ? statusColor : "#334155");
