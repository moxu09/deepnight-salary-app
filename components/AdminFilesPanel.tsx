...
export default function AdminFilesPanel({ apiPath }: { apiPath: string }) {
  ...
  const getToken = useCallback(async () => {
    ...
    if (!data.session) throw new Error("請重新登入後台");
  ...
  const loadFiles = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${apiPath}?category=${category}`, {
  ...
  async function uploadFile() {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.message || "檔案上傳失敗");
  ...
  async function downloadFile(file: StoredFile) {
    ...
    try {
      const token = await getToken();
      const params = new URLSearchParams({ download: file.path });
      ...
      <div className="mx-auto max-w-[1400px] space-y-5">
        <header className="rounded-3xl bg-white p-6 shadow-sm">
          <div className="flex items-center gap-3">
            <span className="rounded-2xl bg-violet-100 p-3 text-violet-600"><FileArchive size={24} /></span>
            <div>
              <h1 className="text-2xl font-black text-slate-900">資料下載</h1>
              <p className="mt-1 text-sm text-slate-500">管理內部營運與員工相關文件。</p>
            </div>
          </div>
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {categories.map(({ key, label, icon: Icon }) => (
              <button key={key} type="button" onClick={() => { setLoading(true); setCategory(key); }} className={`flex items-center gap-3 rounded-2xl border px-5 py-4 text-left font-black transition ${category === key ? "border-violet-300 bg-violet-600 text-white shadow-md shadow-violet-100" : "border-slate-200 bg-slate-50 text-slate-600 hover:border-violet-200 hover:bg-violet-50"}`}>
                <Icon size={20} />{label}
              </button>
            ))}
          </div>
        ...
        {canUpload ? (
          <section className="rounded-3xl border border-violet-100 bg-white p-5 shadow-sm">
            <h2 className="flex items-center gap-2 font-black text-slate-800"><Upload size={19} className="text-violet-500" />上傳到「{category === "operations" ? "營運相關" : "員工相關"}」</h2>
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]">
              <input key={fileInputKey} type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.ppt,.pptx,.txt,.rtf,.jpg,.jpeg,.png,.webp,.gif,.heic,.zip,.rar,.7z" onChange={(event) => setSelectedFile(event.target.files?.[0] || null)} />
              <button type="button" onClick={uploadFile} disabled={!selectedFile || uploading} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-violet-600 px-6 py-3 font-black text-white hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-40">
                {uploading ? <Loader2 size={18} className="animate-spin" /> : <Upload size={18} />}{uploading ? "上傳中…" : "上傳檔案"}
              </button>
            </div>
            <p className="mt-3 text-xs text-slate-500">單一檔案上限 25 MB；僅指定帳號可以上傳。</p>
          </section>
        ) : (
          <p className="rounded-2xl border border-slate-200 bg-white px-5 py-4 text-sm font-semibold text-slate-500">此管理員帳號為查看與下載權限。</p>
        )}
        ...
        <section className="overflow-hidden rounded-3xl bg-white shadow-sm">
          ...
          {loading ? (
            ...
            <p className="p-12 text-center text-sm text-slate-400">這個分類目前沒有檔案</p>
          )}
        </section>
      </div>
    </main>
