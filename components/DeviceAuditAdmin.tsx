    ...
    <section className="rounded-[26px] border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <section className="rounded-2xl border border-sky-200 bg-sky-50 p-5">
          ...
          <ul className="mt-3 space-y-2 text-sm font-semibold leading-6 text-slate-700">
            {(summary.plainLanguage?.details || ["正在產生白話分析，請重新整理報告。"]).map((detail) => (
        ...
        <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
          ...
          <ol className="mt-3 space-y-2 text-sm font-semibold leading-6 text-slate-700">
            {(summary.plainLanguage?.nextSteps || ["請重新整理後再查看建議。"]).map((step, index) => (
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-xs font-black">
        {[
          ...
          <span key={range} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-slate-600">
            {range}｜{label}
          </span>
        ))}
      </div>
