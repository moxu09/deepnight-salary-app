...
export default function WorkReportReviewPanel({
  ...
  async function reviewWithApi(report: WorkReport, action: "approve" | "reject", reason = "") {
    ...
    const response = await fetch(reviewApiPath || `/api/${appKey}/work-reports/review`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${data.session.access_token}` },
      body: JSON.stringify({ id: report.id, action, reason }),
    });
  ...
  async function reject(report: WorkReport) {
    ...
    try {
      ...
      alert(`駁回失敗：${error instanceof Error ? error.message : "未知錯誤"}`);
    } finally {
      setWorkingId("");
    }
  }
