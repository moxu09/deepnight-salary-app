...
begin
  ...
  if p_salary_table not in ('play_orders', 'qiunai_salary_orders') then
    raise exception '不允許的薪資資料表';
  end if;
  if jsonb_typeof(p_staff) <> 'array' then raise exception '陪陪資料格式錯誤'; end if;

  select result into v_existing
  from public.tip_payment_operations
  where operation_key = p_operation_key;
  if found then return v_existing; end if;

  select count(distinct value->>'staff_id') into v_staff_count
  from jsonb_array_elements(p_staff);
  if v_staff_count <= 0 or v_staff_count <> jsonb_array_length(p_staff) then
    raise exception '陪陪資料不可為空或重複';
  end if;
  ...
  if v_balance < v_total then
    raise exception '餘額不足，目前餘額 % ASD，需要 % ASD', v_balance, v_total;
  end if;
  ...
  loop
    ...
    insert into public.play_orders(
      ...
      assigned_player, order_type, order_item, game, service, note,
      channel_id, source_channel_id, price, final_price, paid, paid_at,
      salary_paid, status, completed_at, accepted_at
    ) values (
      p_guild_id, p_user_id, '<@' || p_user_id || '>', '<@' || p_user_id || '>',
      v_staff->>'staff_id', '打賞', p_item, '打賞', '打賞：' || p_item, '打賞',
      p_channel_id, p_channel_id, p_amount, p_amount, true, v_finished_at,
      false, 'completed', v_finished_at, v_finished_at
    ) returning * into v_order;

    if p_salary_table = 'play_orders' then
      update public.play_orders set
        discord_id = v_staff->>'staff_id',
        staff_name = nullif(v_staff->>'staff_name', ''),
        order_amount = p_amount,
        staff_salary = round(p_amount * ((v_staff->>'salary_rate')::numeric / 100)),
        bonus_amount = 0,
        salary_rate = (v_staff->>'salary_rate')::integer,
        salary_level = v_staff->>'salary_level',
        platform_income = p_amount,
        platform_expense = round(p_amount * ((v_staff->>'salary_rate')::numeric / 100)),
        order_finished_at = v_finished_at,
        is_deleted = false
      where id = v_order.id;
    else
      execute format(
        'insert into public.%I (order_id, discord_id, staff_name, customer_name, service_name, order_amount, staff_salary, bonus_amount, salary_rate, salary_level, platform_income, platform_expense, status, order_finished_at, is_deleted) values ($1,$2,$3,$4,$5,$6,$7,0,$8,$9,$6,$7,''未發薪'',$10,false)',
        p_salary_table
      ) using
        coalesce(v_order.order_no, v_order.id::text),
        v_staff->>'staff_id', nullif(v_staff->>'staff_name', ''),
        '<@' || p_user_id || '>', '打賞：' || p_item, p_amount,
        round(p_amount * ((v_staff->>'salary_rate')::numeric / 100)),
        (v_staff->>'salary_rate')::integer, v_staff->>'salary_level', v_finished_at;
    end if;

    v_orders := v_orders || jsonb_build_array(jsonb_build_object(
      'id', v_order.id, 'order_no', v_order.order_no,
      'customer_id', v_order.customer_id, 'assigned_player', v_order.assigned_player,
      'final_price', v_order.final_price, 'service', v_order.service
    ));
  end loop;
  ...
  update public.users set coins = v_balance where user_id = p_user_id;
  insert into public.wallet_logs(user_id, type, amount, balance, note)
  values(p_user_id, '打賞消費', -v_total, v_balance,
    '💝 打賞給 ' || array_to_string(array(select '<@' || (value->>'staff_id') || '>' from jsonb_array_elements(p_staff)), '、') || '｜' || p_item);

  v_existing := jsonb_build_object(
    'operation_key', p_operation_key, 'balance', v_balance,
    'total_amount', v_total, 'orders', v_orders
  );
  insert into public.tip_payment_operations(operation_key, guild_id, user_id, total_amount, result)
  values(p_operation_key, p_guild_id, p_user_id, v_total, v_existing);
  return v_existing;
end;
$function$;

revoke all on function public.pay_tip_with_wallet_atomic(text,text,text,text,integer,text,text,jsonb)
  from public, anon, authenticated;
grant execute on function public.pay_tip_with_wallet_atomic(text,text,text,text,integer,text,text,jsonb)
  to service_role;
