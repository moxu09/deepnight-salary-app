...
export default function StaffPage() {
  ...
  async function loadWithdrawalStatement() {
    ...
    try {
      ...
      const res = await fetch(
        `/api/deepnight/salary-wallet/statement?${params.toString()}`,
        {
          headers: { Authorization: `Bearer ${session.access_token}` },
          cache: "no-store",
        }
      ...
      alert(getErrorMessage(error, "查詢提領紀錄失敗"));
    } finally {
  ...
  async function downloadWithdrawalStatement() {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `深夜不關燈-提領薪資單-${statementFrom}-${statementTo}.pdf`;
      ...
      alert(getErrorMessage(error, "匯出提領薪資單失敗"));
    } finally {
          ...
          {walletLoading && !salaryWallet ? (
            ...
            <>
              <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                ...
                />
                <MiniStat
                  title={`待審核提領（${salaryWallet.totals.pendingRequestCount} 筆）`}
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    ...
                    {salaryWallet.totals.pendingWithdrawn > 0
                      ? `，申請中：${money(
                        ...
                        )}（${salaryWallet.totals.pendingRequestCount} 筆）`
                      : ""}
                  </p>
                </div>
              ...
              <div className="mt-4 rounded-[22px] border border-sky-100 bg-white p-4">
                <h3 className="font-black text-slate-800">每月錢包總額</h3>
                <p className="mt-1 text-xs text-slate-500">
                  此處僅顯示每月總額，個別薪資明細請至訂單詳情查看。
                </p>
                  ...
                  <table className="min-w-full text-sm">
                    <thead className="text-left text-xs text-slate-500">
                      <tr>
                        ...
                        <th className="py-2 pr-4">入帳總額</th>
                        <th className="py-2 pr-4">已提領總額</th>
                        <th className="py-2">待審核</th>
                      </tr>
                    </thead>
                    <tbody>
                      {salaryWallet.monthlySummaries.map((summary) => (
                        <tr key={summary.month} className="border-t border-sky-100">
                          <td className="py-3 pr-4 font-bold text-slate-800">
                            {summary.month.replace("-", " 年 ")} 月
                          </td>
                          <td className="py-3 pr-4 font-black text-emerald-600">
                            +{money(summary.deposited)}
                          </td>
                          <td className="py-3 pr-4 font-black text-slate-700">
                            -{money(summary.withdrawn)}
                          </td>
                          <td className="py-3 font-bold text-amber-600">
                            {money(summary.pendingWithdrawn)}
                          </td>
                        </tr>
                      ))}
                      {salaryWallet.monthlySummaries.length === 0 ? (
                        <tr><td colSpan={4} className="py-4 text-slate-500">目前沒有錢包紀錄</td></tr>
                      ) : null}
                    </tbody>
                  </table>
                </div>
              ...
              <div className="mt-4 rounded-[22px] border border-sky-100 bg-white p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                  ...
                  <div className="grid w-full gap-2 sm:grid-cols-2 lg:w-auto lg:grid-cols-[150px_150px_auto_auto]">
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 rounded-full bg-sky-100 px-4 py-2.5 text-sm font-black text-sky-700 hover:bg-sky-200 disabled:opacity-50"
                    >
                      <Search size={16} />
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 rounded-full bg-sky-500 px-4 py-2.5 text-sm font-black text-white hover:bg-sky-600 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      <FileDown size={16} />
                ...
                {statementData ? (
                  <div className="mt-4">
                    ...
                    <div className="mt-3 overflow-x-auto rounded-[18px] border border-sky-100">
