...
const ORG = "deepnight";
const DEPARTMENT = "深夜不關燈";
const GUILD_ID = process.env.NEXT_PUBLIC_DEEPNIGHT_GUILD_ID || process.env.NEXT_PUBLIC_GUILD_ID || "1501098191813214312";
...
function monthRange(value, offset = 0) {
  ...
  return { start: start.toISOString(), end: end.toISOString(), month: start.toISOString().slice(0, 7) };
}

async function getStaff(discordId) {
  const { data, error } = await supabaseAdmin.from("players").select("discord_id,discord_name,display_name,real_name").eq("discord_id", discordId).maybeSingle();
  if (error || !data) throw new Error("找不到員工資料");
  return data;
}

async function requireAdmin(discordId) {
  const { data } = await supabaseAdmin.from("admins").select("*").eq("discord_id", discordId).eq("is_active", true).maybeSingle();
...
async function priorMonthSalary(discordId, selectedMonth) {
  ...
  const [{ data: orders }, { data: bonuses }] = await Promise.all([
    supabaseAdmin.from("play_orders").select("staff_salary").eq("discord_id", discordId).or(`guild_id.eq.${GUILD_ID},guild_id.is.null`).or("is_deleted.eq.false,is_deleted.is.null").gte("order_finished_at", range.start).lt("order_finished_at", range.end),
    supabaseAdmin.from("players_bonus").select("amount").eq("discord_id", discordId).gte("created_at", range.start).lt("created_at", range.end),
  ]);
...
export async function GET(request) {
  try {
    ...
    const { data: reviewers } = reviewerIds.length
      ? await supabaseAdmin.from("admins").select("*").in("discord_id", reviewerIds)
      : { data: [] };
...
export async function POST(request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("salary_requests").insert({
      organization_code: ORG, guild_id: GUILD_ID, discord_id: discordId, staff_name: name, department: DEPARTMENT,
