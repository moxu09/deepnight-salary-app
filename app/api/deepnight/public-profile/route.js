...
async function getStaff(discordId) {
  const { data, error } = await supabaseAdmin
    .from(STAFF_TABLE)
    .select("discord_id,discord_name,display_name,avatar_url,is_online,can_take_order,is_active,allowed_services")
...
async function seedProfile(staff) {
  const { data, error } = await supabaseAdmin
    ...
    .upsert({
      ...
      display_name: staff.display_name || staff.discord_name || "深夜陪陪",
