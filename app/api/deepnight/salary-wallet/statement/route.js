...
export async function GET(request) {
  try {
    ...
    if (staffError) {
      console.error("[deepnight statement] load staff failed", staffError);
      throw new Error("讀取員工資料失敗");
    }

    if (error) {
      console.error("[deepnight statement] load requests failed", error);
    ...
    const pdf = await createWithdrawalStatementPdf({
      company: "深夜不關燈 We Are Still Here",
      employee: {
        discordId,
        name:
          staff?.display_name ||
    ...
    const filename = `deepnight-salary-withdrawal-${range.from}-${range.to}.pdf`;
