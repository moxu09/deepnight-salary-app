...
export default function AccountingPage() {
  ...
  async function loadReport() {
    ...
    try {
      ...
      const res = await fetch(`/api/deepnight/accounting?month=${month}`, {
        headers: {
  ...
  function exportCsv() {
    ...
    anchor.href = url;
    anchor.download = `deepnight-accounting-${month}.csv`;
    anchor.click();
  ...
  return (
    <main className="min-h-screen bg-[#eef7fd] px-5 py-6 text-slate-900">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="rounded-[32px] border border-sky-100 bg-white p-7 shadow-sm shadow-sky-100">
          <Link
            href="/admin"
          ...
          <div className="mt-5 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-black text-sky-600">Accounting</p>
              <h1 className="mt-2 text-3xl font-black">會計報表</h1>
              <p className="mt-3 text-sm font-semibold leading-7 text-slate-500">
                匯整儲值預收、訂單收入、優惠折扣、薪資獎金、月結應收與提領付款。
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              <input
                ...
                className="rounded-2xl border border-sky-100 bg-white px-4 py-3 text-sm font-bold outline-none focus:border-sky-300"
              />
              <button
                onClick={loadReport}
                className="inline-flex items-center gap-2 rounded-2xl bg-sky-500 px-4 py-3 text-sm font-black text-white shadow-sm disabled:opacity-60"
                disabled={loading}
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-4 py-3 text-sm font-black text-white shadow-sm disabled:opacity-40"
              >
                <Download size={16} />
                匯出 CSV
              </button>
        ...
        {ledgerMissing ? (
          <div className="rounded-3xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-bold leading-7 text-amber-800">
            尚未建立 accounting_ledger 資料表。報表會先用現有訂單與薪資資料產生；
            儲值、月結繳費等新流水要先到 Supabase SQL Editor 執行
            <span className="mx-1 rounded bg-white px-2 py-1">supabase/accounting_ledger.sql</span>
            後才會開始進表。
          </div>
        ) : null}
        ...
        <section className="overflow-hidden rounded-[32px] border border-sky-100 bg-white shadow-sm shadow-sky-100">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[1280px] w-full text-left text-sm">
              <thead className="bg-sky-50 text-xs font-black text-slate-500">
                <tr>
                  <Th>日期</Th>
                  <Th>類型</Th>
                  <Th>分類</Th>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={11} className="px-6 py-12 text-center font-bold text-slate-400">
                      讀取中...
                    </td>
                  </tr>
                  ...
                  displayedRows.map((row) => (
                    <tr key={row.id} className="border-t border-sky-50">
                      ...
                      <Td>
                        <p className="font-black text-slate-800">{row.category}</p>
                        <p className="mt-1 text-xs text-slate-400">{row.source}</p>
                      </Td>
                      <Td>{row.subject || "-"}</Td>
                      <Td>{money(row.amount)}</Td>
                      <Td>
                        <p className="text-emerald-600">入 {money(Math.max(0, row.cash_amount))}</p>
                        <p className="text-rose-500">出 {money(Math.abs(Math.min(0, row.cash_amount)))}</p>
                      </Td>
...
function Td({ children }: { children: React.ReactNode }) {
  return <td className="px-4 py-4 align-top font-semibold text-slate-600">{children}</td>;
}
